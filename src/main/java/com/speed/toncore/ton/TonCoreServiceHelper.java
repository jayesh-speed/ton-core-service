...
public class TonCoreServiceHelper {
	...
	private final String URL_TEMPLATE = "%s%s";
	...
	public BigInteger getTonBalance(String address) {
		...
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_TON_BALANCE), params, getHeaders(tonNode.getApiKey()));
		AccountBalanceDto balance = parseResponse(response, new TypeReference<>() {});
	...
	public String sendMessageWithReturnHash(String messageBoc) {
		...
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.POST,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.SEND_MESSAGE_WITH_RETURN_DATA), null,
				getHeaders(tonNode.getApiKey()), requestBody);
		SendTransactionDto result = parseResponse(response, new TypeReference<>() {});
	...
	public Boolean isDeployed(String address) {
		...
		params.add(JsonKeys.QueryParameters.ADDRESS, address);
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_ACCOUNT_STATE), params, getHeaders(tonNode.getApiKey()));
		AccountStatusDto result = parseResponse(response, new TypeReference<>() {});
		return result.getResult().equalsIgnoreCase(Constants.ACTIVE);
	}
	...
	public WalletInformationDto getWalletInformation(String address) {
		...
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_WALLET_INFORMATION), params, getHeaders(tonNode.getApiKey()));
	...
	public int getSeqNo(String address) {
		try {
			return getWalletInformation(address).getSeqNo();
		} catch (NullPointerException e) {
			throw new RetryException(String.format(Errors.TonIndexer.FAILED_TO_GET_SEQ_NO, address), e);
		}
	}

	@Retryable(retryFor = RetryException.class, backoff = @Backoff(delay = 2000, multiplier = 2, random = true, maxDelay = 8000), maxAttempts = 5)
	public JettonWalletDto getJettonWallet(String ownerAddress, String jettonAddress) {
		TonNode tonNode = tonNodePool.getTonNodeByChainId();
		MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
		params.add(JsonKeys.QueryParameters.OWNER_ADDRESS, ownerAddress);
		params.add(JsonKeys.QueryParameters.JETTON_MASTER_ADDRESS, jettonAddress);
		params.add(JsonKeys.QueryParameters.EXCLUDE_ZERO_BALANCE, Boolean.FALSE.toString());
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_JETTON_WALLET), params, getHeaders(tonNode.getApiKey()));
		return parseResponse(response, new TypeReference<>() {});
	}
	...
	public TonTransactionDto getTransactionByMessageHash(String messageHash) {
		TonNode tonNode = tonNodePool.getTonNodeByChainId();
		MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
		params.add(JsonKeys.QueryParameters.MESSAGE_HASH, messageHash);
		params.add(JsonKeys.QueryParameters.LIMIT, "1");
		params.add(JsonKeys.QueryParameters.OFFSET, "0");
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_TRANSACTION_BY_MESSAGE_HASH), params,
				getHeaders(tonNode.getApiKey()));
		return parseResponse(response, new TypeReference<>() {});
	}
	...
	public FeeDto getEstimateFees(String destinationAddress, String requestBody, String initCode, String initData, boolean ignoreChksig) {
		...
		Map<String, Object> payload = Map.of(JsonKeys.QueryParameters.ADDRESS, destinationAddress, JsonKeys.QueryParameters.BODY, requestBody,
				...
				ignoreChksig);
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.POST,
				String.format(URL_TEMPLATE, tonNode.getBaseUrl(), Endpoints.ChainStack.GET_ESTIMATE_FEES), null, getHeaders(tonNode.getApiKey()),
				payload);
		return parseResponse(response, new TypeReference<>() {});
	}
	...
	public TraceDto getTraceByTraceId(String traceId) {
		TonNode tonNode = tonNodePool.getTonNodeByChainId();
		MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
		String encodedTraceId = URLEncoder.encode(traceId);
		...
		Map<String, Object> response = restClient.executeAPICall(HttpMethod.GET,
				String.format(URL_TEMPLATE, tonNode.getTonCenterUrl(), Endpoints.ChainStack.GET_TRACE_BY_TRACE_ID), params,
				getHeaders(tonNode.getTonCenterApiKey()));
		return parseResponse(response, new TypeReference<>() {});
	}

	private <T> T parseResponse(Map<String, Object> response, TypeReference<T> responseType) {
		...
		if (status.is2xxSuccessful()) {
			if (StringUtil.nullOrEmpty(responseBody)) {
				throw new RuntimeException(Errors.TonIndexer.EMPTY_RESPONSE_BODY);
			}
			try {
				return objectMapper.readValue(responseBody, responseType);
			} catch (JsonProcessingException e) {
				throw new RuntimeException(String.format(Errors.TonIndexer.FAILED_TO_PARSE_RESPONSE, e.getMessage()), e);
			}
		}
		if (status.is4xxClientError()) {
			if (status == HttpStatus.TOO_MANY_REQUESTS) {
				throw new RetryException(Errors.TonIndexer.TO_MANY_REQUEST_ERROR);
			}
		...
		if (status.is5xxServerError()) {
			throw new InternalServerErrorException(String.format(Errors.TonIndexer.INTERNAL_SERVER_ERROR, status.value(), responseBody));
		}
		throw new RuntimeException(String.format(Errors.TonIndexer.UNEXPECTED_ERROR, status.value(), responseBody));
	}
}
